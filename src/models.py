...
def _apply_finetuning(
    base_model,
    unfreeze_prefixes: Iterable[str],
    freeze_bn: bool = True,
) -> None:
...
def _imagenet_input(cfg: Config, preprocess_fn, name: str):
    """Entrada comum dos modelos pré-treinados.

    Os geradores entregam imagens em [0, 1] (idênticas para os três modelos).
    Os pesos ImageNet do Keras, porém, esperam o pré-processamento original de
    cada arquitetura (``preprocess_input``: escala 0-255, RGB->BGR e subtração
    da média da ImageNet). A conversão é feita aqui, dentro do modelo, para
    manter o pipeline de dados idêntico entre as arquiteturas.
    """
...
def build_vgg16(cfg: Config):
    ...
    inputs, x = _imagenet_input(cfg, preprocess_input, "vgg16")
    base = VGG16(weights="imagenet", include_top=False, input_tensor=x)
    _apply_finetuning(
        base,
        TRANSFER_STRATEGY["vgg16"]["unfreeze_prefixes"],
        freeze_bn=True,
    )
...
def build_model(name: str, cfg: Config):
    ...
    if name not in MODEL_BUILDERS:
        raise ValueError(
            f"Modelo desconhecido: {name!r}. Opções: {list(MODEL_BUILDERS)}"
        )
